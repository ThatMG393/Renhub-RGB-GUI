...
function Thread()
	...
	function Threading:TCRGBThread()
		local curThr = {}
	...
	function Threading:BGRGBThread()
		local curThr = {}
...
function UI() -- Call me before :createWindow()

	local GUI = {}

	function GUI:makeWindowDraggable(topObject, mainObject) -- Makes window draggable. Don't use if you don't know what your doing!
		local Dragging = nil
		local DragInput = nil
		local DragStart = nil
	...
	function GUI:createWindow(hubNameText, kbind) -- Create's a window.
		...
		Thread():TCRGBThread():create(UserText):Run()
		...
		UserImage.Image = "https://www.roblox.com/headshot-thumbnail/image?userId="..game.Players.LocalPlayer.UserId.."&width=420&height=420&format=png" -- 420 = noice
		...
		function curWin:addTab(tabName, tabIcon)
			...
			function curTab:addButton(buttonText, buttonCallback) -- Create's a Button in the current tab
				...
				local btnTweenRGB = Thread():TCRGBThread():create(Button)
			...
			function curTab:addToggle() -- Create's a ToggleButton in the current tab
			
			end
			
			function curTab:addDropdown() -- Create's a Dropdown in the current tab
			
			end
 
			function curTab:addColorPicker() -- Create's a ColorPicker in the current tab
				
			end
		return curTab -- Returns currentTab table when called
		end
	return curWin -- Returns currentWindow table when called
	end
return GUI -- Returns GUI table when called
end
...
if true then
	...
	local MainUI = UI() -- You need to call UI() before you call :createWindow() or else it will gonna error out.
    local MGUI = MainUI:createWindow("MG Hub | BF")

    local TestTab = MGUI:addTab("Test tab", "6026568198")
